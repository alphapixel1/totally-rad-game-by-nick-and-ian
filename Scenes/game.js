...
window.onload=function(){
    config.parent=document.getElementById("game_border");
    game=new Phaser.Game(config);
    htmlElements=[...document.querySelectorAll("td"),config.parent,document.querySelector("h1")]
 //   document.querySelector("#cabinet").appendChild(game.canvas)
}
    ...
    setTimeout(function(){
        scene.tweens.addCounter({
            ...
            onUpdate: function (tween)
            {
                ...
                let hex=tint.replace("0x","#");
                htmlElements.forEach(z=>{
                    z.style.borderColor=hex;
                    z.style.color=hex;
                });
                tweenEvent(tint);
            }
...
function loadAssets(scene){
    ...
    scene.load.image("paddle","assets/paddle.png");
    scene.load.image("brick","assets/brick.png");
   /* scene.load.image("Back Arrow","assets/back_arrow.png");
    scene.load.image("Back Arrow Background","assets/back_arrow_background.png");*/
